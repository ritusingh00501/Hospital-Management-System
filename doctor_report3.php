...
$query = "SELECT d_id, name, specilization, date  FROM doctor1 WHERE EXTRACT(YEAR FROM date) = $year";
...
echo"<h1><b><center>Year wise Doctor report<center></b></h1>";
...
$table = "<table id=doctor><thead><tr><th><b>Doctor ID</b></th><th><b>Name</b></th><th><b>Specialization</b></th><th><b>Date</b></th></tr></thead><tbody>";
while ($row = pg_fetch_assoc($result)) {
    $table .= "<tr><td>{$row['d_id']}</td><td>{$row['name']}</td><td>{$row['specilization']}</td><td>{$row['date']}</td></tr>";
}
$table .= "</tbody></table>";
